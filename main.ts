namespace SpriteKind {
    ...
    export const Picos = SpriteKind.create()
    export const dragonss = SpriteKind.create()
}
controller.up.onEvent(ControllerButtonEvent.Pressed, function () {
    if (playerMain.isHittingTile(CollisionDirection.Bottom)) {
        playerMain.vy = 0 - Math.sqrt(2 * (gravedad * alturaSalto))
    }
})
function ataques3 () {
    ataqueMelee = sprites.create(img`
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
    ...
    ataqueIzquierdaAnimacion = []
    for (let value of ataqueDerechaAnimacion) {
        ataqueIzquierdaAnimacion.push(value.clone())
        ataqueIzquierdaAnimacion[ataqueIzquierdaAnimacion.length - 1].flipX()
    }
}
sprites.onOverlap(SpriteKind.Player, SpriteKind.Picos, function (sprite, otherSprite) {
    game.over(false)
})
...
function animaciones () {
    ...
    tiles.placeOnRandomTile(playerMain, assets.tile`myTile5`)
    ...
    for (let value3 of tiles.getTilesByType(assets.tile`myTile`)) {
        spikes = sprites.create(img`
            ...
            .........ccf....
            `, SpriteKind.Picos)
        tiles.placeOnTile(spikes, value3)
...
let ataqueIzquierdaAnimacion: Image[] = []
let ataqueDerechaAnimacion: Image[] = []
let ataqueMelee: Sprite = null
let nivel = 0
let viendoIzquierda = false
let alturaSalto = 0
let gravedad = 0
...
playerMain = sprites.create(img`
    ...
    . . . . . . f f f . . . . . . 
    `, SpriteKind.Player)
controller.moveSprite(playerMain, 80, 0)
scene.cameraFollowSprite(playerMain)
gravedad = 700
alturaSalto = 45
viendoIzquierda = false
info.setLife(15)
playerMain.ay = gravedad
ataques3()
animaciones()
nivel = 0
game.onUpdate(function () {
    if (viendoIzquierda) {
        ataqueMelee.setPosition(playerMain.x - 8, playerMain.y - 0)
    } else {
        ataqueMelee.setPosition(playerMain.x + 8, playerMain.y - 0)
    }
})
forever(function () {
    murcielago = sprites.create(img`
        ...
        . . . . . . . . . . . . . . . . 
        `, SpriteKind.Enemy)
    murcielago.setPosition(randint(scene.cameraProperty(CameraProperty.Left), scene.cameraProperty(CameraProperty.Right)), Math.min(scene.cameraProperty(CameraProperty.Top), 144))
    murcielago.setFlag(SpriteFlag.Ghost, true)
    animation.runImageAnimation(
    murcielago,
    [img`
        . . . . . . . . . . 
    ...
    murcielago.setFlag(SpriteFlag.Ghost, false)
    murcielago.follow(playerMain, 50)
    pause(500)
})
